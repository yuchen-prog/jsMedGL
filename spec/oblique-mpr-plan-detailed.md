...
- [ ] `ObliqueBasis`: 平面基向量 (normal, uAxis, vAxis)，全部归一化且正交
- [ ] `ObliquePlaneComputed`: 计算后的平面参数（含 center, basis, width, height）
- [ ] `ObliqueMPRState`: 三个视图的平面状态 + 共享焦点

#### 1.2 ObliquePlane 类 (`packages/renderer-2d/src/oblique/ObliquePlane.ts`)
- [ ] 构造函数：接收 volume 和 baseOrientation
- [ ] `getBasisForOrientation()`: 返回基准方向的初始基向量（需与现有正交 MPR 一致）
- [ ] `setRotationQuaternion(q: quat)`: 通过四元数设置旋转
- [ ] `setFocalPointRas(ras: vec3)`: 设置共享焦点（RAS 空间）
- [ ] `computeOutputSize()`: 动态计算输出图像尺寸
- [ ] `planeToRas(u, v): vec3`: 平面坐标 → RAS
- [ ] `rasToPlane(ras): {u, v}`: RAS → 平面坐标
- [ ] `planeToIjk(u, v): vec3`: 平面坐标 → IJK（用于采样）
- [ ] `getIntersectionWith(other: ObliquePlaneComputed): Line3D | null`: 两平面交线

#### 1.3 工具函数 (`packages/renderer-2d/src/oblique/math.ts`)
- [ ] `normalizeBasis(basis)`: 确保基向量正交归一化（Gram-Schmidt）
- [ ] `planeIntersection(p1, p2)`: 计算两平面交线
- [ ] `projectBoundingBox(dims, basis)`: 计算体积边界框在平面上的投影尺寸

#### 1.4 单元测试 (`tests/unit/oblique-plane.test.ts`)
- [ ] 基准方向基向量与正交 MPR 一致性
- [ ] 四元数旋转后基向量仍正交归一化
- [ ] planeToRas / rasToPlane 往返一致性
- [ ] planeToIjk 坐标转换正确性
- [ ] 两平面交线计算正确性
- [ ] 输出尺寸计算合理性
...
### 验收标准
- [ ] 所有单元测试通过
- [ ] 基准方向（无旋转）的斜切平面与正交 MPR 渲染结果视觉一致
...
- [ ] 构造函数：接收 NiftiVolume，预处理归一化数据
- [ ] `trilinearSample(ijk: vec3): number`: 三线性插值采样
- [ ] `extractSlice(plane: ObliquePlaneComputed): Uint8Array`: 完整斜切面提取
- [ ] `extractSliceDownsampled(plane, scale): Uint8Array`: 降采样版本（交互预览用）
- [ ] 边界处理：越界采样返回 0
- [ ] window/level 应用

#### 2.2 集成到 WebGLSliceView
- [ ] 新增 `setObliquePlane(plane: ObliquePlaneComputed, extractor: ObliqueExtractor)` 方法
- [ ] 渲染时检测是否有斜切平面，有则用 extractor 提取数据
...
- [ ] 新增 `obliqueState: ObliqueMPRState | null`
- [ ] 新增 `isObliqueMode: boolean` 切换斜切/正交模式
- [ ] 斜切模式下使用 ObliquePlane + ObliqueExtractor

#### 3.2 旋转交互组件
- [ ] 旋转手柄渲染（覆盖在切片视图上）
- [ ] 拖拽旋转逻辑：
  - [ ] mousedown 记录起始位置
  - [ ] mousemove 计算旋转四元数（基于拖拽方向和距离）
  - [ ] mouseup 结束旋转，触发全分辨率渲染
- [ ] 旋转限制（默认 ±60°，可配置）

#### 3.3 性能优化
- [ ] 拖拽过程中使用降采样 (scale=0.25)
- [ ] RAF throttle 限制更新频率
- [ ] mouseup 后切换全分辨率
...
#### 4.1 焦点管理
- [ ] 三个视图共享同一个 focalPoint（IJK/RAS）
- [ ] 拖拽十字线交点更新 focalPoint
- [ ] focalPoint 更新触发三个视图重渲染
